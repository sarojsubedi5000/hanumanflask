...
# Function to detect date columns in the Excel file
...
# Function to clean data by removing duplicates and special characters
def clean_data(df, column_to_check):
    df['count_Row'] = range(1, len(df) + 1)
    df = df.drop_duplicates(subset=[column_to_check], keep='last')
